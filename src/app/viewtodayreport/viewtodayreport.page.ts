...
@Component({
  selector: 'app-viewtodayreport',
  templateUrl: './viewtodayreport.page.html',
  styleUrls: ['./viewtodayreport.page.scss'],
})
export class ViewtodayreportPage implements OnInit {
  ...
  ionViewWillEnter() {
   ...
   this.toaster.doctor_backurl = 'viewtodayreport/'+this.patient_id;
  ...
  get_total_patient_api(patient_id, doctor_id) {
    ...
    this.http
      .get(`${this.url.serverUrl}get_new_patient_report_api?patient_id=${patient_id}&doctor_id=${doctor_id}`)
      .subscribe(
        (res) => {
