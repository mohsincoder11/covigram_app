...
export class ViewreportPage implements OnInit {
  ...
  ionViewWillEnter() {
   this.patient_id = this.route.snapshot.paramMap.get('patient_id');
    this.toaster.doctor_backurl = 'viewreport/'+this.patient_id;
  ...
  get_total_patient_api(patient_id, doctor_id) {
    ...
    this.http
      .get(`${this.url.serverUrl}get_all_report_patient_report_api?patient_id=${patient_id}&doctor_id=${doctor_id}`)
      .subscribe(
        (res) => {
